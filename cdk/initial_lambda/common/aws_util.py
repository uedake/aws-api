...
class S3Access:
    ...
    def _format_url(self, key:str, schema:str="s3") -> str:
        if schema=="s3":
            ...
            return "https://{}.s3.{}.amazonaws.com/{}".fromat(self.bucket,self.region,key)
    ...
    def exist_as_folder(
        ...
        *,
        ...
        summaries=list(self.s3_resource.Bucket(self.bucket).objects.filter(
            Prefix=self.key
        ))
        if len(summaries)==0:
            return False
    ...
    def download_file(self, dest_file_path: str):
        ...
        self.s3_resource.Object(self.bucket, self.key).download_file(
            dest_file_path
        )
    ...
    def upload_one(
        ...
        *,
        ...
        self.s3_resource.Bucket(self.bucket).upload_file(
            src_file_path, Key="{}{}".format(self.key, os.path.basename(src_file_path))
        )
        if delete_after_uploaded:
